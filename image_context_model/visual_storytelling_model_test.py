...
from im2story import configuration
from im2story import visual_storytelling_model


class VisualStorytellingModel(visual_storytelling_model.VisualStorytellingModel):
  """Subclass of ShowAndTellModel without the disk I/O."""

  def build_inputs(self):
    if self.mode == "inference":
      # Inference mode doesn't read from disk, so defer to parent.
      return super(VisualStorytellingModel, self).build_inputs()
    else:
...
class VisualStorytellingModelTest(tf.test.TestCase):

  def setUp(self):
    super(VisualStorytellingModelTest, self).setUp()
  ...
  def testBuildForTraining(self):
    model = VisualStorytellingModel(self._model_config, mode="train")
    model.build()
  ...
  def testBuildForEval(self):
    model = VisualStorytellingModel(self._model_config, mode="eval")
    model.build()
  ...
  def testBuildForInference(self):
    model = VisualStorytellingModel(self._model_config, mode="inference")
    model.build()
